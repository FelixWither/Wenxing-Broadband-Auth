...
8. 如果遇到登陆**不显示**`Success`，尝试重复上面的**步骤1到6**

#### 2.1.2 登出（下线）<u>目前不可用</u>

1. 打开`命令行（cmd）`，将这个程序拖进命令行窗口里

2. 输入一个`空格`，随后输入`logout`，回车

**Note: 科服不知道改了什么，现在登陆宽带以后就不能登出了，只能通过重启<u>登陆了宽带</u>的那台设备来登出**
...
- `logout` 登出宽带
  ...
  **Note: 这个现在跑不通，登陆后再登出会报连接超时，估计是科服把系统改出问题了**
  **正常登陆没问题**
...
Q1: 用了一段时间过后发现登陆宽带不成功，不显示`Success`怎么办？

A1: 尝试重做上面 `2.1.1 登录` 部分的 **步骤1到6**

成因: 由于宿舍的光猫可能会自动更新，网址中的参数可能会因此有变更，所以原本的网址需要更新
